...
cur     = conn.cursor()
cur.execute("SELECT * FROM schedule where title='중식'")
# cur.execute("DELETE FROM schedule where title='중식'")
# cur.execute("DROP TABLE read_notice")
# cur.execute("DROP TABLE notice")
# cur.execute("SELECT * FROM notice")
# cur.execute("INSERT INTO schedule_category(category) values('기타')")
# conn.commit()
print(cur.fetchall())
# print(cur.fetchall())
# ## 인증 쿠키
